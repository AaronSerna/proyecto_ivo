...
export class ComponenteCajaBlancaComponent {
  ...
  formCrearPacientePaso2(formulario: NgForm): void {
    if (formulario.valid) {
      ...
      this.router.navigate(
        ['/administrativo/crearPaciente/crearPacientePaso2/crearPacientePaso3'],
        {
          queryParams: {
            nombre: encodeURIComponent(this.nombre),
            primer_apellido: encodeURIComponent(this.primer_apellido),
  ...
  comprobarDniCrearCita() {
    ...
    this.usuariosService.buscarPorDni().subscribe((data: any) => {
      ...
      if (usuarioEncontrado) {
        ...
        this.router.navigate(
          ['administrativo/crearCita/crearCitaPaso1/crearCitaPaso2'],
          {
            queryParams: {
        ...
        // Aquí puedes guardar el ID del usuario en una variable o realizar otras acciones
      } else {
        //console.log('El DNI no existe.');
        // Manejar el caso en que el DNI no existe, por ejemplo, mostrar un mensaje al usuario.
      }
    });
  }
  ...
  /*
  guardarValueRadiologo(valorSeleccionado: any) {
    console.log(valorSeleccionado);
    this.idRadiologo = valorSeleccionado;
  }
 */

  guardarValueRadiologo(radiologoSeleccionado: any) {
    //console.log(radiologoSeleccionado);
    ...
    if (radiologoSeleccionado) {
      ...
      this.nombreRadiologo = 'No seleccionado';
    }

    // Utiliza directamente el objeto radiologo en lugar de solo el usuario_id
    this.idRadiologo = radiologoSeleccionado ? radiologoSeleccionado.usuario_id : null;

  }

  enviarFormularioPaso2() {
    const fecha = this.crearCitaPaso2Formulario.fecha;
    const hora = this.crearCitaPaso2Formulario.hora;
    const radiologo = this.crearCitaPaso2Formulario.radiologo;

    this.crearCitaService.guardarCitaPaso2(this.crearCitaPaso2Formulario);
    this.router.navigate(
      [
        '/administrativo/crearCita/crearCitaPaso1/crearCitaPaso2/crearCitaPaso3',
      ],
      {
        queryParams: {
          id: encodeURIComponent(this.id),
          dni: encodeURIComponent(this.dni),
          fecha: encodeURIComponent(this.crearCitaPaso2Formulario.fecha),
          hora: encodeURIComponent(this.crearCitaPaso2Formulario.hora),
